...
export function buildSystemPrompt(ctx: SpaceContextForPrompt): string {
  ...
  return [
    ...
    'Numeric discipline (mandatory):',
    '- Money fields from tools look like { formatted, minor, currency }.',
    '- Copy `formatted` into the answer exactly as returned — do not round, re-format, or recalculate.',
    '- When stating a total, also include the raw minor integer once as `(minor: 12345)` so figures stay verifiable.',
    '- Counts and rates: copy the exact integer/decimal from the tool.',
