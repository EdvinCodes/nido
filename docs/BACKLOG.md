...
- [ ] Wire transaction composer `edit` mode (form currently only opens for `create`/`scan`; `openEdit` is a stub) — deferred from product polish because amount edits hit the `update_transaction` balance-trigger bug. Target: ledger hardening after RPC fix.
