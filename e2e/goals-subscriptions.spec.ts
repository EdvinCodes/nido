...
test.describe('goals and subscriptions', () => {
  test('lists seeded goals and opens detail', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /metas|goals/i })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /fondo de emergencia/i })).toBeVisible();
  });

  test('shows annualised subscription total and opens detail', async ({ page }) => {
    const spaceId = await signInDemo(page);
    await page.goto(`/s/${spaceId}/subscriptions`);
    await expect(page.getByRole('heading', { name: /suscripciones|subscriptions/i })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /netflix/i })).toBeVisible();
  });
